...
export async function syncVariantInventoryLevels( productGid, shop, accessToken )
{
  ...
  console.info( `syncing productGid = ${productGid}` )
  ...
  if( 'true' !== shouldSync )
  {
    console.info( `Skipping variant sync: ${productGid}` )
    return
  }
  ...
  console.info( `Synced ${variants.length} variants to quantity: ${minAvailable}` )
}
