...
export async function syncVariantInventoryLevels( productGid, shop, accessToken )
{
  ...
  console.dir( product.data, {depth: null} )
  console.log( '---------- /end productRes ' )

  // const shouldSync = product.data?.product?.metafield?.value
  // if( 'true' !== shouldSync )
  // {
  ...
  for (const { node: variant } of variants)
  {
    ...
    inventoryLevels.forEach( ({node: level}) => {
      ...
      levelsToUpdate.push( {itemId: variant.inventoryItem.id, locationId: level.location.id} )
    })
  }
